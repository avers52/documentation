...
Полный промпт для двенадцатипозиционной модели трехузлового кластера. Модель 12/3 (12 состояний марковской модели \ 3 узла кластера)
...
- работоспособными являются только три состояния: S3, S2, S1.

## 1.1 Состояния модели

### Работоспособные состояния (только три)
...
Все остальные состояния модели — неработоспособные.
...
- S_latent — скрытый отказ одного из работоспособных узлов (единое агрегированное состояние);
...
Считаем, что кластер в состояниях S3_tf, S2_tf, S1_tf, S_latent, S3_failover, S2_failover, S2_failback, S1_failback, S0_fail находится в состоянии отказа (не предоставляет требуемую функцию).
...
## 1.3 Переходы для трехузловой модели
...
- S1 → S0_fail: λη (явный отказ единственного работоспособного узла);
...
## 1.5 Временный сбой (Transient fault)
...
Среднее время перезапуска: Ttr.

Интенсивность восстановления после временного сбоя: μ_tr = 1 / Ttr.

---

## 1.6 Интенсивности и средние времена
...
2. Укажи работоспособные и неработоспособные состояния. Явно перечисли работоспособные состояния (S3, S2, S1) и неработоспособные состояния (S0_fail, S3_tf, S2_tf, S1_tf, S_latent, S3_failover, S2_failover, S2_failback, S1_failback).
3. Построй граф состояний (Mermaid), если он уже задан в условии — повтори.
4. Составь матрицу генератора Q размером 12×12.
5. Выведи стационарные уравнения баланса для всех 12 состояний.
6. Найди стационарные вероятности аналитически (вырази все вероятности через P3 и знаменатель D).
7. Рассчитай:
   7.1. Стационарный коэффициент готовности (формулу представь в явном виде через все параметры модели: λ, μ, λ_tr, μ_tr, μ_failover, μ_failback, θ, η).
8. Выполни численный расчет с точностью не менее 10 значащих цифр.
...
- Направление графа: слева направо (LR). При этом по мере деградации состояний основная цепочка должна располагаться как S3 → S2 → S1 → S0_fail, а вспомогательные состояния (S3_tf, S2_tf, S1_tf, S_latent, S3_failover, S2_failover, S2_failback, S1_failback) могут располагаться над или под основной цепочкой;
- Подписи переходов оформляй в кавычках: "интенсивность", т.к. могут встречаться скобки, которые будут давать ошибки.
...
Важные требования к LaTeX-формулам для GitHub Markdown:

- Все формулы должны быть заключены в $$...$$ для блочных формул и $...$ для inline-формул.
- Избегай многострочных формул с переносами строк внутри $$...$$. Каждая формула должна быть записана в одну строку.
- Не используй \left( и \right) внутри формул; используй обычные скобки ().
- Не используй \cdot внутри дробей; для умножения используй явное обозначение или просто ставь множители рядом.
- Избегай \dfrac; используй \frac.
- Не разбивай одну формулу на несколько строк с помощью \\ внутри $$...$$.
...
- Легенда состояний;
- Группы состояний;
- Основные формулы (Unicode + LaTeX);
- Численный результат;
- Ограничения модели.

## 3.5 Требования к точности вычислений

- Все промежуточные вычисления выполняй с точностью не менее 12 значащих цифр.
- Окончательные результаты представляй с точностью не менее 10 значащих цифр.
- Избегай преждевременного округления промежуточных результатов.
- При вычислении знаменателя D явно перечисли все слагаемые и их численные значения.
- Проверь нормировку: сумма всех стационарных вероятностей должна равняться 1 с точностью не менее 1e-10.
...
- MTBF одного узла = 30000 часов (нагруженный резерв);
- MTTR одного узла = 24 часа (одна ремонтная бригада);
- среднее время failover = 30 секунд;
- среднее время failback = 90 секунд;
- среднее время перезапуска после временного сбоя T_tr = 180 секунд;
- η = 0,99;
- среднее время обнаружения скрытого отказа = 8 часов;
- среднее время между временными сбоями одного узла = 1 год (8760 часов);
